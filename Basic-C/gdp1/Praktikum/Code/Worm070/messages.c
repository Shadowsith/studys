...
void showStatus(struct board* aboard, struct worm* aworm) {
    ...
    int pos_line2 = LINES -ROWS_RESERVED;
    int pos_line3 = LINES -ROWS_RESERVED + 1;
    int pos_line4 = LINES -ROWS_RESERVED + 2;
    int pos_line5 = LINES -ROWS_RESERVED + 3;
    ...
    mvprintw(pos_line4, 1,"Debugging Headindex: %3d", aworm->headindex );
    mvprintw(pos_line5, 1,"Debugging Maxindex : %3d", aworm->maxindex);
}
...
int showDialog(char* prompt1, char* prompt2) {
    ...
    clearLineInMessageArea(pos_line4);
    clearLineInMessageArea(pos_line5);
